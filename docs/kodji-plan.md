...
| **PR-Z** | Flutterwave billing | Provider-agnostic subscriptions, webhook adapter, XOF integer pricing. **Built 11 Sep 2026 (#93): pay-per-period (1 / 12 months) via hosted checkout, verify-before-activate, extend-not-restart, 7d/1d reminders + expiry job, `/billing`. No auto-renew — Flutterwave payment plans are card-only, which would exclude mobile money. Sandbox-verified on both Flutterwave and Paystack (`BILLING_PROVIDER`); Flutterwave declined sub-$5M CI merchants 11 Sep 2026, so live keys ride on Paystack's business verification (RCCM + tax id needed).** |
| **PR-AA** | PWA shell and Web Push | Manifest, service worker, push subscriptions; Discord demoted to ops-only. **Shipped 12 Sep 2026:** installable shell (manifest + icons + `/sw.js` that caches only the static shell and an `/offline` page, never data), RFC 8291/8292 Web Push written on `cryptography` with the RFC test vector pinned, `push_subscriptions` (migration 0022, per user/device), `/alerts` "Enable on this device", delivery fans out per member device with email as the fallback for members with no device (the iOS-not-installed case). `just vapid-keygen` once per deployment. |
